...
internal class DefaultWifiApiImplEnableWifiTest(
    ...
    fun test() {
        ...
        given(mockWifiManager.setWifiEnabled(true)).willReturn(params.enableWifiResult)
        ...
        assertEquals(params.enableWifiResult, result)
    }
    ...
    companion object {
        ...
        fun paramValues(): List<EnableWifiParams> {
            return listOf(
                EnableWifiParams(enableWifiResult = true),
                EnableWifiParams(enableWifiResult = false)
            )
        }

        data class EnableWifiParams(val enableWifiResult: Boolean)
    }
}
