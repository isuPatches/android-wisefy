...
internal class DefaultWifiAdapterTest {
    ...
    fun disableWifi() {
        // Given
        given(mockApi.disableWifi()).willReturn(true)
        val adapter = DefaultWifiAdapter(wifiManager = mockWifiManager, api = mockApi)
    ...
    fun disableWifi_unableToDisable() {
        // Given
        given(mockApi.disableWifi()).willReturn(false)
        val adapter = DefaultWifiAdapter(wifiManager = mockWifiManager, api = mockApi)
    ...
    fun disableWifi_assertion_default() {
        ...
        // Then
        verify(mockApi, never()).disableWifi()
        assertEquals(
            DisableWifiResult.Failure.Assertion(AssertionMessages.Wifi.ANDROID_29_REQUEST_USED_ON_PRE_ANDROID_29),
            result,
        )
    }
    ...
    fun enableWifi() {
        // Given
        given(mockApi.enableWifi()).willReturn(true)
        val adapter = DefaultWifiAdapter(wifiManager = mockWifiManager, api = mockApi)
    ...
    fun enableWifi_unableToEnable() {
        // Given
        given(mockApi.enableWifi()).willReturn(false)
        val adapter = DefaultWifiAdapter(wifiManager = mockWifiManager, api = mockApi)
    ...
    fun enableWifi_assertion_default() {
        ...
        // Then
        verify(mockApi, never()).enableWifi()
        assertEquals(
            EnableWifiResult.Failure.Assertion(AssertionMessages.Wifi.ANDROID_29_REQUEST_USED_ON_PRE_ANDROID_29),
            result,
        )
    }
