 ...
 * @author Patches
 */
internal class WiseFyPrechecksTests : BaseAndroidJUnit4TestClass() {
    ...
    @Test fun addNetworkPrechecks_withPassword_failure_nullSSIDParam() {
    ...
    @Test fun addNetworkPrechecks_withPassword_failure_networkAlreadySaved() {
        mockWiseFySearchUtil.isNetworkASavedConfiguration(true)
        assertEquals(NETWORK_ALREADY_CONFIGURED, wisefyPrechecks.addNetworkPrechecks(WEP_NETWORK_SSID, WEP_NETWORK_PASSWORD).code)
    }

    @Test fun addNetworkPrechecks_withPassword_success() {
        mockWiseFySearchUtil.isNetworkASavedConfiguration(false)
        assertEquals(DEFAULT_PRECHECK_RETURN_CODE, wisefyPrechecks.addNetworkPrechecks(WEP_NETWORK_SSID, WEP_NETWORK_PASSWORD).code)
    }

    @Test fun connectToNetworkPrechecks_failure_nullParam() {
        assertEquals(MISSING_PARAMETER, wisefyPrechecks.connectToNetworkPrechecks(null).code)
    }
    @Test fun connectToNetworkPrechecks_success() {
        assertEquals(DEFAULT_PRECHECK_RETURN_CODE, wisefyPrechecks.connectToNetworkPrechecks(TEST_SSID).code)
    }

    @Test fun disableWifiChecks() {
        assertEquals(DEFAULT_PRECHECK_RETURN_CODE, wisefyPrechecks.disableWifiChecks().code)
    }

    @Test fun disconnectFromCurrentNetworkChecks() {
        assertEquals(DEFAULT_PRECHECK_RETURN_CODE, wisefyPrechecks.disconnectFromCurrentNetworkChecks().code)
    }

    @Test fun enableWifiChecks() {
    ...
    @Test fun searchForSSIDsChecks_success() {
