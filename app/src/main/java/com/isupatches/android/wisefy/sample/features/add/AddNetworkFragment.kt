...
internal class AddNetworkFragment : BaseFragment(), AddNetworkView {
    ...
    private val addNetworkResult = registerForActivityResult(
    ...
    private val addOpenNetworkPermissionsLauncher =
    ...
    private val addWPA2NetworkPermissionsLauncher =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { results ->
            if (results.all { it.value }) {
                addWPA2Network()
            } else {
    ...
    private val addWPA3NetworkPermissionsLauncher =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { results ->
            if (results.all { it.value }) {
                addWPA3Network()
            } else {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.addNetworkBtn.setOnClickListener {
            when (binding.addNetworkTypeRdg.checkedRadioButtonId) {
                R.id.openNetworkRdb -> {
                    addOpenNetworkPermissionsLauncher.launch(arrayOf(ACCESS_FINE_LOCATION, CHANGE_WIFI_STATE))
                }
                R.id.wpa2NetworkRdb -> {
                    addWPA2NetworkPermissionsLauncher.launch(arrayOf(ACCESS_FINE_LOCATION, CHANGE_WIFI_STATE))
                }
                R.id.wpa3NetworkRdb -> {
                    addWPA3NetworkPermissionsLauncher.launch(arrayOf(ACCESS_FINE_LOCATION, CHANGE_WIFI_STATE))
                }
            }
        }
